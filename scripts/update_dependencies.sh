...
cd $REPO_ROOT
set +e
$BAZEL_DEPS_PATH generate -r $REPO_ROOT -s third_party/maven_dependencies.bzl -d third_party/maven_dependencies.yaml
RET_CODE=$?
set -e
...
if [ $RET_CODE == 0 ]; then
  ...
  echo "Failure, checking out third_party/java"
  cd $REPO_ROOT
  git checkout third_party/java third_party/maven_dependencies.bzl
  exit $RET_CODE
fi

$BAZEL_DEPS_PATH format-deps -d $REPO_ROOT/third_party/maven_dependencies.yaml -o
